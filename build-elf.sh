...
# param 2: build type (Debug/Release) - default Debug
...
docker run --rm \
    ...
    "${IMAGE}" \
    bash -c "cmake '${DOCKER_SRC_DIR}/external/prime2-practice-mod' -DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DRANDOMIZER=ON -G Ninja && cmake --build . --config ${BUILD_TYPE}"
